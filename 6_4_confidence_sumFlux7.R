# This script calculates the confidence interval for the summation of multiple fluxes
...
# specify reactions whose fluxes are to be summed up

# Lac <-> Pyr
f1 <- reaction_data %>% filter(reactions == "Ile.hp->AcCoA.Lv") %>% pull(enzyme); f1
f2 <- reaction_data %>% filter(reactions == "Leu.hp->AcCoA.Lv") %>% pull(enzyme); f2
f3 <- reaction_data %>% filter(reactions == "Lys.hp->AcCoA.Lv") %>% pull(enzyme); f3
f4 <- reaction_data %>% filter(reactions == "Thr.hp->AcCoA.Lv") %>% pull(enzyme); f4
f5 <- reaction_data %>% filter(reactions == "Phe.hp->AcCoA.Lv") %>% pull(enzyme); f5
f6 <- reaction_data %>% filter(reactions == "Trp.hp->AcCoA.Lv") %>% pull(enzyme); f6
f7 <- reaction_data %>% filter(reactions == "Tyr.hp->AcCoA.Lv") %>% pull(enzyme); f7


# f.all <- list(c(f1, f2), c(f5, f6), c(f9, f10), c(f11, f12))
f.all <- list(c(f1, f2, f3, f4, f5, f6, f7)) # more c(fx, fy, fz) can be included in the list()
...
func.calculate_confidence_interval <- function(f){
  ...
  flux.C.I_index1 <- f[1] # reaction index of fluxes to sum up
  flux.C.I_index2 <- f[2]
  flux.C.I_index3 <- f[3]
  flux.C.I_index4 <- f[4]
  flux.C.I_index5 <- f[5]
  ...
  # optimal value of the sum of fluxes
  flux.C.I_value.optimal <- 
    optimal_allFlux[flux.C.I_index1] + 
    optimal_allFlux[flux.C.I_index2] +
    optimal_allFlux[flux.C.I_index3] +
    optimal_allFlux[flux.C.I_index4] +
    optimal_allFlux[flux.C.I_index5] +
    optimal_allFlux[flux.C.I_index6] +
    optimal_allFlux[flux.C.I_index7] 
  ...
  d.flux.cost <- tibble(flux.index = paste0(
    
    flux.C.I_index1, "+", flux.C.I_index2, "+", flux.C.I_index3, "+", 
    flux.C.I_index4, "+", flux.C.I_index5, "+", flux.C.I_index6, "+", flux.C.I_index7),
  ...
  while (binary.index <= total.binary.steps) { 
    ...
    message("sum of 7 fluxes ", 
            "v", f[1], "+v", f[2], "+v", f[3], "+v", f[4], "+v", f[5],"+v", f[6],"+v", f[7],
            "; stage ", stage, "; binary index ", binary.index)
    ...
    for (optim.step in 1:n.opt.steps) { 
      ...
      for (tracer.k in tracers){
        message("sum of fluxes ", "v", f[1], "+v", f[2], "+v", f[3], "+v", f[4], "+v", f[5], "+v", f[6], "+v", f[7], "; stage ", stage, "; binary index ", binary.index, "; optim.step ", optim.step,   "; tracer ", tracer.k)
      ...
      Amat <- rbind(   Amat0[flux.C.I_index1, ] + Amat0[flux.C.I_index2, ] + Amat0[flux.C.I_index3, ] + Amat0[flux.C.I_index4, ] + Amat0[flux.C.I_index5, ] + Amat0[flux.C.I_index6, ] + Amat0[flux.C.I_index7, ] , Amat)
      Amat <- rbind(-( Amat0[flux.C.I_index1, ] + Amat0[flux.C.I_index2, ] + Amat0[flux.C.I_index3, ] + Amat0[flux.C.I_index4, ] + Amat0[flux.C.I_index5, ] + Amat0[flux.C.I_index6, ] + Amat0[flux.C.I_index7, ]), Amat)
      
      bo   <- c(   optimal_allFlux[flux.C.I_index1] + optimal_allFlux[flux.C.I_index2] + optimal_allFlux[flux.C.I_index3] + optimal_allFlux[flux.C.I_index4] + optimal_allFlux[flux.C.I_index5] + optimal_allFlux[flux.C.I_index6] + optimal_allFlux[flux.C.I_index7] + h,    bo)
      bo   <- c(-( optimal_allFlux[flux.C.I_index1] + optimal_allFlux[flux.C.I_index2] + optimal_allFlux[flux.C.I_index3] + optimal_allFlux[flux.C.I_index4] + optimal_allFlux[flux.C.I_index5] + optimal_allFlux[flux.C.I_index6] + optimal_allFlux[flux.C.I_index7] + h)-1, bo)     
      ...
      if  ((optimal_allFlux[flux.C.I_index1] + optimal_allFlux[flux.C.I_index2] + optimal_allFlux[flux.C.I_index3] + optimal_allFlux[flux.C.I_index4] + optimal_allFlux[flux.C.I_index5] + optimal_allFlux[flux.C.I_index6] + optimal_allFlux[flux.C.I_index7] + h) < myMin) {
    ...
    d.flux.cost.h <- tibble(flux.index = paste0(flux.C.I_index1, "+", flux.C.I_index2, "+", flux.C.I_index3, "+", flux.C.I_index4, "+", flux.C.I_index5, "+", flux.C.I_index6, "+", flux.C.I_index7),
  ...
  save(d.flux.cost, 
       ...
       file = paste0("./CI/", 
                     flux.C.I_index1, "+", flux.C.I_index2, "+", flux.C.I_index3, "+", 
                     flux.C.I_index4, "+", flux.C.I_index5, "+", flux.C.I_index6, "+", 
                     flux.C.I_index7, ".RData"))
...
message("\n\nCompleted running confidence interval for sum of multiple fluxes!\n\n")
