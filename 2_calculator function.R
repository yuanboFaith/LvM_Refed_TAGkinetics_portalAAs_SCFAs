...
func.stoicEMU <- function(
  ...
  # myTracer= "13ChpSCFAa"  ;   tracerInputRate = 0
  # myTracer= "13CLinoKinC" ;   tracerInputRate = 0
  # combine 
  ...
  portalSCFAs <- c("Acetate.hp", "Propionate.hp", "Butyrate.hp")
  ...
  for (name in x) {
    print(name)
    # if (name == "Propionate.hp_123+CO2.Blood_1") stop()
  ...
  if (myTracer %>% str_detect("13ChpAA")) {
    for (i in 1:length(l.EMU.substrates)){
      ...
      if (i_EMU %>% str_detect("\\.hp") &   
          ...
          ! (str_detect(i_EMU, "Acetate") | str_detect(i_EMU, "Propionate") | str_detect(i_EMU, "Butyrate"))   ) {
  ...
  if (myTracer %>% str_detect("13ChpSCFA")) {
    for (i in 1:length(l.EMU.substrates)){
      ...
      if ( str_detect(i_EMU, "Acetate") | str_detect(i_EMU, "Propionate") | str_detect(i_EMU, "Butyrate") ) {
...
# func.stoicEMU(myTracer = "13ChpSCFAa",       tracerInputRate = 0)
# func.stoicEMU(myTracer = "13ChpAAb",       tracerInputRate = 0)
# func.stoicEMU(myTracer = "13CLinoKinA",       tracerInputRate = 0)
