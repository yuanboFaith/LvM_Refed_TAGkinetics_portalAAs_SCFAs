...
f.all <- list(c(f1, f2)) # more c(fx, fy, fz) can be included in the list()
...
func.calculate_confidence_interval <- function(f){
  ...
  while (binary.index <= total.binary.steps) { 
    ...
    message("sum of Acetate and Butyrate fluxes ", 
            "v", f[1], "+v", f[2], "; stage ", stage, "; binary index ", binary.index)
    ...
    for (optim.step in 1:n.opt.steps) { 
      ...
      for (tracer.k in tracers){
        message("sum of fluxes ", "v", f[1], "+v", f[2], "; stage ", stage, "; binary index ", binary.index, "; optim.step ", optim.step,   "; tracer ", tracer.k)
  ...
  save(d.flux.cost, 
       ...
       reaction_data,
       file = paste0("./CI/", 
                     flux.C.I_index1, "+", flux.C.I_index2, ".RData"))
