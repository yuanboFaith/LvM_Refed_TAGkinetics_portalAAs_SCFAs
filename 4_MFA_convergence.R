...
if (Sys.info()[["sysname"]] == "Darwin") { # if Bo's Mac, use 6 cores
  n.cores <- makeCluster(6, outfile = "" )  
} else {
  n.cores <-min(length(tracers), 30)       # lab's computer, use a max of 30 threads
}

message(n.cores, " cores used.")
cl <- makeCluster(n.cores, outfile = "" )  # use at max 28 cores
...
# obs vs sim
p.btm <- p02.facet  + facet_wrap(~tracer, nrow = 2)  + 
  theme(legend.position = "bottom") +
  guides(color = guide_legend(nrow = 2, override.aes = list(size = 5, fontface = "bold"))) 

cowplot::plot_grid(p.top, p.btm, rel_heights = c(3, 2), ncol = 1)
