# This script calculate confidence interval for specified reactions
...
# find exchange equillibrium reactions
rmv1 <- reaction_data %>% filter(str_detect(reactions, "Mal") & str_detect(reactions, "OAA"))
rmv2 <- reaction_data %>% filter(str_detect(reactions, "Suc") & str_detect(reactions, "Mal"))
rmv3 <- reaction_data %>% filter(str_detect(reactions, "Suc") & str_detect(reactions, "Suc2"))
rmv4 <- reaction_data %>% filter(str_detect(reactions, "Ala") & str_detect(reactions, "Pyr.c"))
rmv5 <- reaction_data %>% filter(str_detect(reactions, "Pyr.c") & str_detect(reactions, "Lac"))
rmv6 <- reaction_data %>% filter(str_detect(reactions, "GAP") & str_detect(reactions, "DHAP")) %>% filter(!str_detect(products, "Glc"))
rmv <- rbind(rmv1, rmv2, rmv3, rmv4, rmv5, rmv6)

reaction_data_no.equilibrium <- reaction_data %>% anti_join(rmv) 



# find blood glucose associated fluxes
f.Glc.Blood <- reaction_data_no.equilibrium %>% filter(str_detect(reactions, "Glc.Blood")) %>% pull(enzyme)

# find liver, kidney, muscle associated fluxes
f.Lv <- reaction_data_no.equilibrium %>% filter(str_detect(reactions, "\\.Lv")) %>% pull(enzyme)
f.M  <- reaction_data_no.equilibrium %>% filter(str_detect(reactions, "\\.M"))  %>% pull(enzyme)

# get the total fluxes of interest
f.all <- c(f.Lv) %>% unique() %>% sort() # note that some duplicated fluxes of interest need to be removed

message(length(f.all), " reactions of interested selected")



# check with reactions have already been calculated, and exclude them from f.all
CI.files.detected <- list.files("./CI",  full.names = TRUE) # all CI files, including net difference and sum of fluxes
CI.files.done <- CI.files.detected[!CI.files.detected %>% str_detect("[+-]")] # remove net and sum of fluxes; keep only single flux
CI.done <- CI.files.done %>% str_extract_all("\\d{1,3}", simplify = T) %>% c() # extract index of completed single reactions
message(length(CI.done), " reactions' CI have been calculated, with .RData files found")

f.all <- f.all[!f.all %in% CI.done]
message(length(f.all), " reactions of interested remaining to be calculated")
...
# Create a cluster
cl <- makeCluster( min (length(f.all), detectCores() ), outfile = "log_CI.txt")  
...
func.calculate_confidence_interval <- function(f){
  ...
  flux.C.I_index <- f # index of the flux
  flux.C.I_value.optimal <- optimal_allFlux[flux.C.I_index] # optimal value of the flux
  ...
  d.flux.cost <- tibble(flux.index = flux.C.I_index,
  ...
  while (binary.index <= total.binary.steps) { 
    ...
    message("reaction #", f, "; stage ", stage, "; binary index ", binary.index)
    ...
    for (optim.step in 1:n.opt.steps) { 
      ...
      for (tracer.k in tracers){
        message("reaction #", f, "; stage ", stage, "; binary index ", binary.index, "; optim.step ", optim.step,   "; tracer ", tracer.k)
        ...
        # manual check
        # l.Compound.tissue.seq[[tracer.k]][!(l.Compound.tissue.seq[[tracer.k]] %in% names(l.EMU.sources)) ]
      ...
      Amat <- rbind( Amat0[flux.C.I_index, ], Amat)
      Amat <- rbind(-Amat0[flux.C.I_index, ], Amat)
      bo   <- c(  optimal_allFlux[flux.C.I_index] + h,    bo)
      bo   <- c(-(optimal_allFlux[flux.C.I_index] + h)-1, bo)
      ...
      if  ((optimal_allFlux[flux.C.I_index] + h) < myMin) {
      ...
      if (optim.step >= 5) {
        cost_latest5iterations <- cost.history[(optim.step - 3) : optim.step]
    ...
    d.flux.cost.h <- tibble(flux.index = flux.C.I_index,
  ...
  save(d.flux.cost, 
       ...
       file = paste0("./CI/", flux.C.I_index, ".RData"))
} 
...
message("\n\nCompleted running confidence interval analysis!\n\n")
